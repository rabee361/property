import { useEffect, useState } from 'react';
import { useTranslation } from 'react-i18next';
import { Link, useLocation, useNavigate } from 'react-router-dom';
import { FaCalendarAlt, FaGlobe, FaIdCard, FaUser } from 'react-icons/fa';
import heroBg from '../assets/hero.jpg';
import brandIcon from '../assets/icon.webp';

const OwnerCompleteInfo = () => {
  const { t, i18n } = useTranslation();
  const navigate = useNavigate();
  const location = useLocation();
  const accountData = location.state?.accountData;
  const [formData, setFormData] = useState({
    idCardImage: null,
    fullName: '',
    nationalIdNumber: '',
    birthday: '',
  });
  const [idCardPreview, setIdCardPreview] = useState('');

  useEffect(() => {
    return () => {
      if (idCardPreview) {
        URL.revokeObjectURL(idCardPreview);
      }
    };
  }, [idCardPreview]);

  const handleChange = (e) => {
    setFormData({ ...formData, [e.target.name]: e.target.value });
  };

  const handleIdCardChange = (e) => {
    const file = e.target.files?.[0];

    if (!file) {
      return;
    }

    const nextPreview = URL.createObjectURL(file);

    if (idCardPreview) {
      URL.revokeObjectURL(idCardPreview);
    }

    setIdCardPreview(nextPreview);
    setFormData({
      ...formData,
      idCardImage: file,
    });
  };

  const handleSubmit = (e) => {
    e.preventDefault();

    console.log('Owner profile completed:', {
      ...accountData,
      ...formData,
      idCardImage: formData.idCardImage?.name ?? '',
    });

    navigate('/owner');
  };

  const toggleLanguage = () => {
    const nextLang = i18n.language === 'en' ? 'ar' : 'en';
    i18n.changeLanguage(nextLang);
  };

  return (
    <div className="relative min-h-screen flex items-center justify-center">
      <img
        src={heroBg}
        alt=""
        className="absolute inset-0 w-full h-full object-cover"
      />

      <div className="absolute inset-0 bg-linear-to-b from-black/70 via-black/50 to-black/70"></div>

      <button
        onClick={toggleLanguage}
        className="absolute top-6 right-6 rtl:right-auto rtl:left-6 z-20 flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 backdrop-blur-md text-white hover:bg-white/20 transition-all duration-300 cursor-pointer"
      >
        <FaGlobe className="w-4 h-4" />
        <span className="text-xs uppercase tracking-widest">
          {i18n.language === 'en' ? 'العربية' : 'English'}
        </span>
      </button>

      <div className="relative z-10 w-full max-w-md mx-4 my-8">
        <div className="bg-white/40 backdrop-blur-sm rounded-2xl shadow-2xl p-8 md:p-10">
          <div className="flex justify-center mb-6">
            <Link to="/">
              <img src={brandIcon} alt="Brand" className="h-12 w-auto" />
            </Link>
          </div>

          <h1 className="text-2xl font-light text-white text-center uppercase tracking-widest mb-2">
            {t('auth.owner_info_title')}
          </h1>
          <p className="text-white/80 text-sm text-center mb-8">
            {t('auth.owner_info_subtitle')}
          </p>

          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label
                htmlFor="owner-id-card"
                className="block cursor-pointer rounded-xl border border-dashed border-white/35 bg-black/15 p-4 text-white transition-colors hover:border-brand-gold hover:bg-black/25"
              >
                <div className="flex items-center gap-3">
                  <div className="flex h-11 w-11 items-center justify-center rounded-full bg-white/10">
                    <FaIdCard className="h-5 w-5" />
                  </div>
                  <div>
                    <p className="text-xs uppercase tracking-widest text-white/70">
                      {t('auth.id_card_image')}
                    </p>
                    <p className="mt-1 text-sm font-medium">
                      {formData.idCardImage ? formData.idCardImage.name : t('auth.upload_id_card')}
                    </p>
                    <p className="mt-1 text-xs text-white/60">
                      {t('auth.id_card_help')}
                    </p>
                  </div>
                </div>

                {idCardPreview && (
                  <img
                    src={idCardPreview}
                    alt={t('auth.id_card_image')}
                    className="mt-4 h-40 w-full rounded-lg object-cover"
                  />
                )}
              </label>
              <input
                id="owner-id-card"
                type="file"
                accept="image/*"
                onChange={handleIdCardChange}
                required
                className="hidden"
              />
            </div>

            <div className="relative">
              <FaUser className="absolute top-1/2 -translate-y-1/2 left-4 rtl:left-auto rtl:right-4 text-white/60 w-4 h-4" />
              <input
                type="text"
                name="fullName"
                placeholder={t('auth.full_name')}
                value={formData.fullName}
                onChange={handleChange}
                required
                className="w-full pl-11 rtl:pl-4 rtl:pr-11 pr-4 py-3 bg-transparent text-white placeholder:text-white/60 border border-white/30 rounded-lg text-sm focus:outline-none focus:border-brand-gold focus:ring-1 focus:ring-brand-gold/30 transition-colors"
              />
            </div>

            <div className="relative">
              <FaIdCard className="absolute top-1/2 -translate-y-1/2 left-4 rtl:left-auto rtl:right-4 text-white/60 w-4 h-4" />
              <input
                type="text"
                name="nationalIdNumber"
                placeholder={t('auth.national_id_number')}
                value={formData.nationalIdNumber}
                onChange={handleChange}
                required
                className="w-full pl-11 rtl:pl-4 rtl:pr-11 pr-4 py-3 bg-transparent text-white placeholder:text-white/60 border border-white/30 rounded-lg text-sm focus:outline-none focus:border-brand-gold focus:ring-1 focus:ring-brand-gold/30 transition-colors"
              />
            </div>

            <div className="relative">
              <FaCalendarAlt className="absolute top-1/2 -translate-y-1/2 left-4 rtl:left-auto rtl:right-4 text-white/60 w-4 h-4" />
              <input
                type="date"
                name="birthday"
                value={formData.birthday}
                onChange={handleChange}
                required
                className="w-full pl-11 rtl:pl-4 rtl:pr-11 pr-4 py-3 bg-transparent text-white border border-white/30 rounded-lg text-sm focus:outline-none focus:border-brand-gold focus:ring-1 focus:ring-brand-gold/30 transition-colors scheme-dark"
              />
            </div>

            <button
              type="submit"
              className="w-full py-3 bg-brand-gold text-white text-xs uppercase tracking-widest-extra font-semibold rounded-lg hover:bg-brand-gold/90 transition-all duration-300 mt-2 cursor-pointer"
            >
              {t('auth.save_and_continue')}
            </button>
          </form>

          <p className="text-center text-sm text-white/80 mt-6">
            <Link to="/signup" className="text-brand-gold font-semibold hover:underline">
              {t('auth.back_to_signup')}
            </Link>
          </p>
        </div>
      </div>
    </div>
  );
};

export default OwnerCompleteInfo;